...
function estimatePrepTime(ingridientsArray,prepTime){
  return ingridientsArray.length * prepTime || ingridientsArray.length * 2
}
...
function calculateQuantities(ingredientsList, numberOfLayers){
 ...
 if (ingredientsList.includes("sugar")){
  ingridients['sugar'] = numberOfLayers*100
 }
 if (ingredientsList.includes("eggs")){
  ingridients['eggs'] = numberOfLayers*2
 }
  return ingridients
}
...
function improveRecipe(object,numberOfPortions){
 const ingridients = object
 if (object.hasOwnProperty('eggs')) { ingridients['eggs'] = ingridients['eggs'] * numberOfPortions }
 if (object.hasOwnProperty('milk')) { ingridients['milk'] = ingridients['milk'] * numberOfPortions }
 if (object.hasOwnProperty('sugar')) { ingridients['sugar'] = ingridients['sugar'] * numberOfPortions }
 if (object.hasOwnProperty('flour')) { ingridients['flour'] = ingridients['flour'] * numberOfPortions }
 return ingridients
}
